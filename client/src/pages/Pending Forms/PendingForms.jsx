...
import {
  pendingForms,
  pendingFormsAdmin,
  pendingFormsAccountSection,
  pendingFormsPurchaseSection,
  pendingFormsRegistrar,
  approveFormRequest,
  approveFormAccountSectionRequest,
  approveFormPurchaseSectionRequest,
  approveFormRegistrarRequest,
  rejectFormHODRequest,
  rejectFormAccountSectionRequest,
  rejectFormPurchaseSectionRequest,
  rejectFormRegistrarRequest,
} from "../../services/Apis";
import { ToastContainer, toast } from "react-toastify";
import { useState, useEffect } from "react";
import { useDisclosure, Stack, Skeleton, Alert, AlertIcon, AlertTitle, AlertDescription, Image, HStack} from "@chakra-ui/react";
...
// Function to fetch pending forms for the specified user ID
...
const fetchPendingFormsForAdmin = async (department, email, jwtToken) => {
  try {
    const data = {
      department,
      email
    ...
    const response = await pendingFormsAdmin(data, header);
    if (response.status === 200) {
      return { status: response.status, data: response.data };
    }
    else {
...
const fetchPendingFormsForAccountSection = async (role, jwtToken) => {
  try {
    const data = {
      role
    };
    ...
    const response = await pendingFormsAccountSection(data, header);
    if (response.status === 200) {
      return { status: response.status, data: response.data };
    }
    else {
...
const fetchPendingFormsForPurchaseSection = async (role, jwtToken) => {
  try {
    const data = {
      role
    };
    ...
    const response = await pendingFormsPurchaseSection(data, header);
    if (response.status === 200) {
      return { status: response.status, data: response.data };
    }
    else {
...
const fetchPendingFormsForRegistrar = async (role, jwtToken) => {
  try {
    const data = {
      role
    };
    ...
    const response = await pendingFormsRegistrar(data, header);
    if (response.status === 200) {
      return { status: response.status, data: response.data };
    }
    else {
...
export default function PendingForms() {
  ...
  const [approvedBy, setApprovedBy] = useState("NONE");
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(0);
  const [filteredData, setFilteredData] = useState([]);
  const [loadingRow, setLoadingRows] = useState({});
  ...
  const handleApproveForm = async (id) => {
    ...
    if (role === "HOD") {
      ...
      try {
        ...
        const response = await approveFormAccountSectionRequest(data, header);
        if (response.status === 200) {
          const successMessage = response.data.message;
          showToast("Approved",successMessage, 'success');
      ...
      try {
        ...
        const response = await approveFormPurchaseSectionRequest(data, header);
        if (response.status === 200) {
          const successMessage = response.data.message;
          showToast("Approved",successMessage, 'success');
          navigate("/approvedForms");
      ...
      try {
        ...
        const response = await approveFormRegistrarRequest(data, header);
        if (response.status === 200) {
          const successMessage = response.data.message;
          showToast("Approved",successMessage, 'success');
          navigate("/approvedForms");
  ...
  const handleRejectForm = async () => {
    ...
    if (role === "HOD") {
      ...
      try {
        ...
        const response = await rejectFormAccountSectionRequest(data, header);
        if (response.status === 200) {
          const successMessage = response.data.message;
          showToast("Rejected",successMessage, 'success');
          navigate("/rejectedForms");
      ...
      try {
        ...
        const response = await rejectFormPurchaseSectionRequest(data, header);
        if (response.status === 200) {
          const successMessage = response.data.message;
          showToast("Rejected",successMessage, 'success');
          navigate("/rejectedForms");
      ...
      try {
        ...
        const response = await rejectFormRegistrarRequest(data, header);
        if (response.status === 200) {
          const successMessage = response.data.message;
          showToast("Rejected",successMessage, 'success');
          navigate("/rejectedForms");
  ...
  useEffect(() => {
    if (successMessage) {
      ...
      setSuccessMessage("");
      navigate("/pendingForms", { replace: true, state: {} });
    }
  }, [successMessage]);

  useEffect(() => {
    if (role === "HOD") {
      fetchPendingFormsForAdmin(department, email, jwtToken)
        .then((res) => {
          if (res.status === 200) {
      ...
      fetchPendingFormsForAccountSection(role, jwtToken)
        .then((res) => {
          if (res.status === 200) {
            setUserPendingForms(res.data);
          }
      ...
      fetchPendingFormsForPurchaseSection(role, jwtToken)
        .then((res) => {
          if (res.status === 200) {
            setUserPendingForms(res.data);
          }
      ...
      fetchPendingFormsForRegistrar(role, jwtToken)
        .then((res) => {
          if (res.status === 200) {
            setUserPendingForms(res.data);
          }
      ...
      {error && (
        <div className="mt-40">
          ...
          >
            ...
            <AlertDescription mt={4} fontWeight={"semibold"} fontSize={"medium"}>
              {error.description}
            </AlertDescription>
          </Alert>
        </div>
                  ...
                  <thead>
                    <tr>
                      {TABLE_HEAD.map((head) => (
                        <th
                          key={head}
                          className="border-2 border-blue-gray-100 text-center text-white bg-orange-400 p-4"
                        >
                          <Typography
                            variant="large"
                            color="white"
                      ...
                      {/* <th className="border-b border-blue-gray-00 text-lg text-white bg-orange-400 p-4">
                        Actions
                      </th> */}
                    </tr>
                  </thead>
                  <tbody>
                    {currentRows.length > 0 && currentRows.map(({ id, name, budgetHead, date, formCategory }, index) => (
                      ...
                      >
                        ...
                        {(role === "HOD" || role === "AO" || role === "AR" || role === "JR" || role === "DR") && (
                          <td className="p-4 flex justify-center gap-1">
                              <Button
                                variant={'outline'}
                                p={2}
                        ...
                        {(role === "ACCOUNTANT" || role === "JAO" || role === "JS") && (
                          <td className="p-4 flex justify-center gap-1">
                            <Button
                              variant={"outline"}
                              p={2}
                            ...
                            <Button
                              ...
                              mr={2}
                              type="button"
                              onClick={() => navigate(`/pendingForm/${id}`)}
                            >
                              Edit & Approve
                            </Button>
                ...
                {totalPages > 1 && (
                  <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
                    <TailwindButton
                      ...
                      className="flex justify-center items-center text-lg gap-1"
                    >
